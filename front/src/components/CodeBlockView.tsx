...
export function CodeBlockView({ code, language, filename, onCopy, onApply, onDownload, incomplete, isDark }: CodeBlockViewProps) {
  ...
  return (
    <div className={`rounded-lg overflow-hidden border ${isDark ? 'bg-gray-900' : 'bg-gray-50'} ${incomplete ? 'border-blue-600/50' : (isDark ? 'border-gray-700' : 'border-gray-300')}`}>
      {/* 标题栏 */}
      <div className={`flex items-center justify-between px-4 py-2 border-b ${isDark ? 'bg-gray-800' : 'bg-gray-100'} ${incomplete ? 'border-blue-600/50' : (isDark ? 'border-gray-700' : 'border-gray-300')}`}>
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs px-2 py-0.5 rounded ${incomplete ? 'text-blue-400 bg-blue-900/50' : (isDark ? 'text-gray-500 bg-gray-700' : 'text-gray-600 bg-gray-300')}`}>{language}</span>
        </div>
        <div className="flex items-center gap-1 group">
          {onApply && (
            <div className="relative">
              ...
              <span className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-200 text-gray-700'}`}>
                对比差异
              </span>
            </div>
          )}
          ...
          <code className="font-mono">
            {lines.map((line, index) => (
              <div key={index} className="flex">
                <span className={`select-none w-10 text-right pr-4 flex-shrink-0 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>
                  {index + 1}
                </span>
                <span
