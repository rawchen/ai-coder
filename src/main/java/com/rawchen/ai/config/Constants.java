package com.rawchen.ai.config;

/**
 * @author RawChen
 * @date 2023-11-22 09:57
 */
public class Constants {

    public final static String SECRET_KEY = "rawchen";

    public final static String DEEPSEEK_API_KEY = "sk-xxx";
    public final static String DEEPSEEK_BASE_URL = "https://api.deepseek.com";
    public final static String DEEPSEEK_CHAT_MODEL = "deepseek-chat";
    public final static String DEEPSEEK_REASONER_MODEL = "deepseek-reasoner";

    public final static String KIMI_API_KEY = "sk-xxx";
    public final static String KIMI_BASE_URL = "https://api.moonshot.cn/v1";
    public final static String KIMI_MODEL = "kimi-k2.5";

    public final static String GLM_API_KEY = "xxx";
    public final static String GLM_BASE_URL = "https://open.bigmodel.cn/api/paas/v4";
    public static final String GLM_MODEL = "glm-4.7";
}
